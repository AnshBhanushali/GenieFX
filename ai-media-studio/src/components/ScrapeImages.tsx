"use client";

import { useState } from "react";
import { scrapeImages } from "@/lib/api";
import { useTaskPolling } from "@/hooks/useTaskPolling";
import { motion } from "framer-motion";

export function ScrapeImages() {
  const [url, setUrl] = useState("");
  const [taskId, setTaskId] = useState<string | null>(null);
  const { data: taskResult, isLoading } = useTaskPolling(taskId);

  const handleScrape = async () => {
    const data = await scrapeImages(url);
    setTaskId(data.task_id);
  };

  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="bg-gray-800/50 p-6 rounded-xl border border-gray-700"
    >
      <h2 className="text-xl font-semibold mb-4">Scrape Images</h2>
      <input
        type="url"
        value={url}
        onChange={(e) => setUrl(e.target.value)}
        placeholder="Enter URL"
        className="w-full p-2 mb-4 bg-gray-900 border border-gray-600 rounded focus:outline-none focus:border-cyan-500"
      />
      <button
        onClick={handleScrape}
        className="w-full bg-gradient-to-r from-cyan-500 to-blue-500 text-gray-900 font-bold py-2 rounded hover:shadow-lg hover:shadow-cyan-500/50 transition"
      >
        Scrape
      </button>
      <div className="mt-4">
        {isLoading && <p className="flex items-center"><Spinner /> Scraping...</p>}
        {taskResult?.status === "SUCCESS" && (
          <ul className="list-disc pl-5 text-cyan-400">
            {taskResult.result.images.map((img: string) => (
              <li key={img}>
                <a href={img} target="_blank" rel="noopener noreferrer" className="underline">
                  {img}
                </a>
              </li>
            ))}
          </ul>
        )}
        {taskResult?.status === "FAILURE" && (
          <p className="text-red-400">Error: {taskResult.error}</p>
        )}
      </div>
    </motion.div>
  );
}

const Spinner = () => (
  <div className="w-5 h-5 border-2 border-t-cyan-500 rounded-full animate-spin mr-2" />
);