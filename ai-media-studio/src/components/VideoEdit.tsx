...
export function VideoEdit() {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (droppedFile && droppedFile.type.startsWith("video/")) setFile(droppedFile);
  };
  ...
  const handleEdit = async () => {
    if (!file) return;
    const data = await videoEdit(prompt, file);
    ...
    >
      ...
      >
        ...
        <input
          ...
          id="video-upload"
        />
        <label htmlFor="video-upload" className="cursor-pointer text-cyan-400 underline">
          Upload
        </label>
      ...
      <div className="mt-4">
        ...
        {taskResult?.status === "SUCCESS" && (
          <div>
            <video controls src={taskResult.result.output_path} className="w-full rounded" />
            <a
